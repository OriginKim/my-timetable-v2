    ...
    return (
        <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', backgroundColor: '#f4f7f9' }}>
          ...
          <div style={{ display: 'flex', gap: '20px', marginTop: '40px' }}>
            <button onClick={() => setCampus('seoul')} style={{ padding: '20px 40px', background: '#3f51b5', color: 'white', border: 'none', borderRadius: '15px', cursor: 'pointer', fontWeight: 'bold' }}>서울</button>
            <button onClick={() => setCampus('cheonan')} style={{ padding: '20px 40px', background: '#ff5722', color: 'white', border: 'none', borderRadius: '15px', cursor: 'pointer', fontWeight: 'bold' }}>천안</button>
          </div>
        </div>
    );
  }
      ...
      <div style={{ padding: isMobile ? '10px' : '20px', maxWidth: '1200px', margin: '0 auto', fontFamily: 'Pretendard, sans-serif' }}>
        <header style={{ textAlign: 'center', marginBottom: '20px' }}>
          ...
          <div style={{ marginTop: '10px', fontSize: '1.2rem', fontWeight: 'bold' }}>
            신청 학점: <span style={{ color: '#3f51b5' }}>{totalCredits}</span> / 21
          </div>
          <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', marginTop: '15px', flexWrap: 'wrap' }}>
            <input type="text" placeholder="학번" value={studentId} onChange={(e) => setStudentId(e.target.value)} style={{ padding: '10px', borderRadius: '8px', border: '1px solid #ddd', width: isMobile ? '100%' : '150px' }} />
            <button onClick={() => alert("저장/불러오기 기능은 학번 입력 후 사용하세요.")} style={{ padding: '10px 15px', background: '#3f51b5', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>불러오기</button>
            <button style={{ padding: '10px 15px', background: '#4caf50', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>저장</button>
            <button onClick={async () => {
              const canvas = await html2canvas(timetableRef.current, { backgroundColor: '#fff', scale: 2 });
              const link = document.createElement('a');
              link.download = `timetable_${studentId || 'my'}.png`;
              link.href = canvas.toDataURL();
              link.click();
            }} style={{ padding: '10px 15px', background: '#000', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>이미지</button>
          </div>
        </header>

        {/* 💡 13교시 추가: 반복문 배열에 13을 포함시켰습니다. */}
        ...
        <section style={{ background: '#fff', padding: '15px', borderRadius: '15px', marginBottom: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fit, minmax(120px, 1fr))', gap: '10px' }}>
            <input type="text" placeholder="검색" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ padding: '10px', borderRadius: '8px', border: '1px solid #ddd' }} />
            ...
            <select value={selectedCredit} onChange={(e) => setSelectedCredit(e.target.value)} style={{ padding: '10px', borderRadius: '8px', border: '1px solid #ddd' }}>
              ...
              {[1,2,3].map(c => <option key={c} value={String(c)}>{c}학점</option>)}
            </select>
              ...
              return (
                  <tr key={c.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    <td style={{ padding: '12px 15px' }}>
                      ...
                      <div style={{ fontSize: '11px', color: '#64748b' }}>{c.dept} | {c.professor} | {c.credit}학점</div>
                    </td>
        ...
        {isModalOpen && (
            <div style={{ position: 'fixed', top:0, left:0, width:'100%', height:'100%', backgroundColor:'rgba(0,0,0,0.5)', display:'flex', justifyContent:'center', alignItems:'center', zIndex: 1000 }}>
              <div style={{ backgroundColor:'white', padding:'20px', borderRadius:'20px', width:'95%', maxWidth:'400px' }}>
                <h3 style={{ marginTop: 0 }}>🕒 시간대 필터</h3>
                ...
                <button onClick={() => setIsModalOpen(false)} style={{ width: '100%', padding: '12px', background: '#4f46e5', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer' }}>필터 적용 및 닫기</button>
              </div>
            </div>
        )}
      </div>
